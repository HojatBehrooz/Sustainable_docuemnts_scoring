...
structure by adding space instead of any removed characters and the words. 
it helps to keep track the cleaned text be related to the orginal positioning of the
corpus. it helps to find the orginal text postion by knowing the cleaned postion.
it also dehyphanates the word in orginal corpus.
"""
...
def dehyphen(rawText):
    ...
    while(i<len(rawText)):
        ...
        try:
            if(rawText[i:i+2]=='-\n'):
                index_next_part=0
                for j in range(i+2,len(rawText)):
                    if(rawText[j]!=' '):
                        if(rawText[j].isalpha()):
                            index_next_part=j
                            break
                if(j==len(rawText)-1):
                    break
                if(index_next_part!=0):
                    for k in range(index_next_part,len(rawText)):
                        if(rawText[k].isalpha()):
                            continue
                        else:
                            break
                    subq=rawText[index_next_part:k]+" "*(index_next_part-i)
                    rawText=rawText[:i]+subq+rawText[i+len(subq):]
                    i=i+len(subq)
                    continue
            i+=1
        except:
...
def ngramming_bforce2 (rawText, stopwords, word_len, ngram_range, 
                       bigram_base_frequency, trigram_base_frequency):
    """
    ...
    The threshold for the ngrams is calculated as
        ...
        That makes the frequency a linear function of the length of the document

    """
    #change HYphanated words to the orginal format
    rawText=dehyphen(rawText)
    #add a space on starting and end of raw text
    rawText=" "+rawText+" "
...
#    print ('\n---The input files contains', len(counts), 'words')
    num_words=len(in_file.split())
    bigram_freq_calc = num_words**bigram_base_frequency
    trigram_freq_calc =num_words**trigram_base_frequency
...
def ngramming_bforce_bench (bench_list,rawText, stopwords, word_len, ngram_range, 
                       bigram_base_frequency, trigram_base_frequency):
    """
    This function is a frequency-based n-grammer.
    It also clean the right and left boundaries from stopwords.
    ...
    count_values = vec_fit_array.sum(axis=0)
    vocab = vectorizer.vocabulary_
    # the following  create a list of tuples (frequency, word)
    counts = sorted([(count_values[i],k) for k,i in vocab.items()], reverse=True)
#    print ('\n---The input files contains', len(counts), 'words')
    num_words= len(in_file.split())
    bigram_freq_calc = num_words**bigram_base_frequency
    ...
    # the following is calculating the number of words in the ngrams in the data structure
    df['ngram_type'] = df['Name'].str.split().str.len()
    # find the ngrams which has frequency more than the minimums and also 
    # those have entry in benchmark
    ngrams_list = df[((df['ngram_type'] ==ngram_range[0]) &
                       (df.Frequency >bigram_freq_calc)) | # for bigrams
                      ((df['ngram_type'] ==ngram_range[1]) &
                       (df.Frequency >trigram_freq_calc)) | # for trigrams
                      (df.Name.str.split().isin(bench_list)) #for benchmarks
                     ]['Name'].values.tolist()
    if(len(df[(df.Name.str.split().isin(bench_list)) #for benchmarks
                     ]['Name'].values.tolist())!=0):
        print("following benchmarks ngrams keeped",df[(df.Name.str.split().isin(bench_list)) #for benchmarks
                         ]['Name'].values.tolist())
#    print ('- Before cleaning there are', len(ngrams_list))
    # ignore ngrams which include stopwords in lef and right side
    clean_ngrams_list = right_left_clean (ngrams_list,stopwords)
